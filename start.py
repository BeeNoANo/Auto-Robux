from list.keyauth import api

import sys, time, platform, os, hashlib, getpass
from time import sleep
from datetime import datetime
from item.filter_information import getInfo, info, Gruop
from index import login
import item.terminal as terminal
...
keyauthapp = api(
    name = "RBTHASK",
    ownerid = "2QXKYvCpda",
    secret = "a7362291a5641a2ace278d4c8c6b4950bab1a07378588827fa625b437b25c469",
    version = "1.0",
...
def answer():
    try:
        ...
        if ans == "1":
            ...
            user = input('Provide username: ')
            password = getpass.getpass('Provide password: ')
            license = getpass.getpass('Provide License: ')
            ...
            key = getpass.getpass('Provide License: ')
...
while True:
    ...
    if choice == '1':
        ...
        for line in lines:
            i = getInfo(line)
            login(i.id, i.name, i.username, i.password, i.robux, i.money, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), i.robux)

    elif choice == '2':
        # Check account

        terminal.Effect.load(1)
        terminal.Account.CheckAccount()
        pass
    elif choice == '4':
        # Gruop account
        terminal.Effect.load(1)
        ...
        with open("..\data.txt", "r", encoding="utf-8") as f:
            found_tax = False  # Biến cờ để theo dõi trạng thái 

            for line in f:
                if "thuế" in line:
                    Gruop.PrintG()
                    terminal.Effect.Print(Fore.GREEN + "Gruop Acc Duplicates is Done\n" + "CHECK ROBUX PLEASE ENTER VALUE 2"  + Style.RESET_ALL)
                    found_tax = True
                    break  # Chỉ thoát vòng lặp khi đã tìm thấy "Thuế" 

            if not found_tax:  # In thông báo lỗi nếu không tìm thấy "Thuế"
                terminal.Effect.Print(Fore.RED + "Gruop Acc Duplicates already Done\n" + "CHECK AGAINT PLEASE" + Style.RESET_ALL)
        pass
    elif choice == '5':
        pass
    elif choice == '6':
